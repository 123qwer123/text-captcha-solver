...
async def fetch(url):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, ssl=False, timeout=timeout, headers=headers) as response:
            ...
            print(f"[fetch] status={response.status}, len={len(data)}")
            return data

# 图片读取操作
async def set_imageSource(dataType, imageSource):
    if dataType == 1:
        # 兼容 data:image 格式（油猴可能错误发来 base64 图）
        if imageSource.startswith('data:image'):
            b64_data = imageSource.split(',', 1)[-1]
...
async def run(item):
    ...
    res = cap_model.run_dict(imageSource, prompt=item.prompt)
